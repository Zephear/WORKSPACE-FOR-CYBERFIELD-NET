...
function setAuthShellVisibility(isAuthenticated) {
  ...
  if (loginSection)
    loginSection.style.display = isAuthenticated ? "none" : "flex";
  if (mainNav) mainNav.style.display = isAuthenticated ? "flex" : "none";
  if (mainContent)
    mainContent.style.display = isAuthenticated ? "block" : "none";
  if (logoutItem && logoutItem.parentElement) {
    logoutItem.parentElement.style.display = isAuthenticated ? "" : "none";
  }
}
...
function syncUserBadge() {
  ...
  if (roleEl && currentUser) {
    ...
    roleEl.className = "user-info-role role-" + currentUser.role;
  }
}
...
async function handleSaveNews() {
  ...
  try {
    ...
    if (editingNewsId !== null) {
      res = await fetch(apiUrl(`/api/news/${editingNewsId}`), {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      res = await fetch(apiUrl("/api/news"), {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
...
async function loadNotes() {
  if (!currentUser) return;
  try {
    const res = await fetch(apiUrl(`/api/notes/${currentUser.user_id}`));
    if (res.ok) {
...
async function handleSaveNote() {
  ...
  try {
    ...
    if (editingNoteId) {
      res = await fetch(apiUrl(`/api/notes/${editingNoteId}`), {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          title,
